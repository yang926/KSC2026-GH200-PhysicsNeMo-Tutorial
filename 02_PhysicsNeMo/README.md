...
참가자는 노트북을 순서대로 재생하는 데 그치지 않습니다. 물리 실험값, 해석해 코드, ODE 잔차 목표값, FFT 직접해법, FNO 모드 수, 학습 단계 수를 직접 수정하고 예측·학습·비교·해석까지 수행합니다.
...
- [ ] 선택한 `fno_modes`와 학습 단계 수를 기록했습니다.
- [ ] `final_state_test_metrics.json`과 테스트 예측 그림을 확인했습니다.
- [ ] 학습 전후 상대 L2 오차, 전체 실행 시간, 최대 PyTorch GPU 메모리를 기록했습니다.
