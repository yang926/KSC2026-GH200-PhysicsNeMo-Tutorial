...
필수 Poisson FNO 실습을 마친 참가자는 강사 안내 후 이 실습을 진행할 수 있습니다. 실행 전에 정확도와 계산 비용의 변화를 예상하고, 실행 뒤 실제 비율을 계산해 가설과 비교합니다.
...
두 실험은 같은 축소 데이터셋과 난수 시드 `2026`을 사용합니다. 모델의 모드 수와 결과 폴더 이름만 다르다는 검사를 통과한 뒤 실행합니다. 관찰 결과는 이 데이터·모델·학습 설정에 대한 값으로 기록합니다.
