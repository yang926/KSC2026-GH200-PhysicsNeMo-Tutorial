...
4. **로컬 터미널 2**를 새로 열고, 화면의 `[1/2]` 아래에 표시된 `ssh -N ...` 한 줄을 통째로 붙여 넣습니다. OTP와 비밀번호를 입력해 **`KSC2026 터널에 정상 접속되었습니다.`**가 표시되면 연결된 상태입니다. 이후 이 터미널은 터널을 유지하므로 프롬프트가 돌아오지 않습니다. 실습 중에는 창을 열어 둡니다.
