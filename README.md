...
이메일로 안내받은 PILOT SSH 명령으로 로그인한 뒤 공용 명령을 실행하면, Slurm이 현재 사용 가능한 계산 노드의 NVIDIA GH200 한 개를 배정합니다. 로컬 터미널과 웹 브라우저를 준비하세요.

1. **로컬 터미널 1**에서 이메일로 안내받은 PILOT SSH 명령을 실행하고 OTP와 비밀번호를 입력합니다.
...
- PILOT 로그인까지 끊겼다면 이메일의 SSH 명령으로 다시 로그인한 뒤 공용 명령을 실행합니다.
