...
사용자는 계산 노드, 포트, 계정 또는 로그인 호스트를 직접 채우지 않습니다. 공용 런처가 Slurm의 실제 배정 결과를 확인한 뒤 완성된 SSH 명령을 한 줄로 출력합니다. 인증이 완료되면 명령이 정상 접속 메시지를 표시하며, 이후 `ssh -N`이 터널을 유지하므로 터미널 프롬프트는 돌아오지 않습니다.
