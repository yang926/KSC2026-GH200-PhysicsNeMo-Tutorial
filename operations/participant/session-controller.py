...
# squeue가 이 사유로 실패하면 "확인할 수 없다"가 아니라 "그 Job은 없다"는 확정 답입니다.
# 끝난 Job은 MinJobAge(기본 300초)가 지나면 컨트롤러 기록에서 빠지고, 그때부터
# squeue --jobs=<id>는 exit 0 + 빈 출력이 아니라 exit 1 + 이 메시지를 냅니다.
JOB_ABSENT_RE = re.compile(r"invalid job id", re.IGNORECASE)
HEX40_RE = re.compile(r"^[0-9a-f]{40}$")
HEX64_RE = re.compile(r"^[0-9a-f]{64}$")
# 한 노드는 GH200 4장에 Grace 코어 288개입니다. 한 사람이 GPU 한 장을 쓰므로
# 1인 몫은 72개인데, 4명이 동시에 잡아도 노드에 32개가 남도록 64로 잡았습니다.
# site.env가 비어 있으면 Slurm 기본값인 1 CPU가 배정되고, 그러면 01-1 5절의
# 스레드 스케일링 관찰이 성립하지 않습니다. site.env에 값이 있으면 그 값이 우선합니다.
...
def query_job(job_id: str) -> dict[str, str] | None:
    ...
    if process.returncode:
        detail = process.stderr.strip() or process.stdout.strip() or f"exit {process.returncode}"
        # 기록에서 빠진 Job은 없는 Job으로 봅니다. 그래야 cmd_start가 STALE로 판정해
        # archive_stale()로 옛 기록을 치우고 새 Job을 시작할 수 있습니다. 이 한 가지
        # 사유만 통과시키고, 컨트롤러 장애처럼 판정이 불가능한 경우는 그대로 막습니다.
        if JOB_ABSENT_RE.search(detail):
            return None
...
def build_sbatch(
    ...
    cpus = config.get("KSC_CPUS_PER_TASK", "") or DEFAULT_CPUS_PER_TASK
