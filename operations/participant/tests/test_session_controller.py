...
class UnifiedSessionControllerTests(unittest.TestCase):
    ...
    def build_command(self, root, **overrides):
        config_path = root / "site.env"
        config_path.write_text("", encoding="utf-8")
        config = {
            "KSC_IMAGE": "/scratch/hackathon/ksc2026/images/course.sif",
        ...
        return MODULE.build_sbatch(
            config, config_path, "edu001",
            {"state": root / "state", "logs": root / "logs"},
            Path("/scratch/edu001/ksc2026/workspaces/course-aaaaaaaaaaaa"),
            self.course(root),
        )

    def test_sbatch_requests_the_per_gpu_cpu_share_when_the_site_leaves_it_blank(self) -> None:
        """A blank site.env used to fall through to Slurm's 1-CPU default."""
        with tempfile.TemporaryDirectory() as temporary:
            command = self.build_command(Path(temporary))
        self.assertEqual(MODULE.DEFAULT_CPUS_PER_TASK, "64")
        self.assertIn("--cpus-per-task=64", command)

    def test_site_config_cpu_value_still_overrides_the_default(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            command = self.build_command(Path(temporary), KSC_CPUS_PER_TASK="36")
        self.assertIn("--cpus-per-task=36", command)
        self.assertNotIn("--cpus-per-task=64", command)

    def test_query_job_treats_purged_job_as_absent_but_still_blocks_on_scheduler_failure(self) -> None:
        """Slurm forgets a finished job after MinJobAge; that is 'gone', not 'unknown'."""
        purged = subprocess.CompletedProcess(
            args=["squeue"], returncode=1, stdout="",
            stderr="slurm_load_jobs error: Invalid job id specified\n",
        )
        with mock.patch.object(MODULE, "run", return_value=purged):
            self.assertIsNone(MODULE.query_job("19172"))
        ...
        for stderr in (
            ...
            with self.subTest(stderr=stderr):
                failure = subprocess.CompletedProcess(
                    args=["squeue"], returncode=1, stdout="", stderr=stderr
                )
                with mock.patch.object(MODULE, "run", return_value=failure):
                    with self.assertRaises(MODULE.SessionError):
                        MODULE.query_job("19172")

    def test_start_recovers_from_a_job_slurm_no_longer_remembers(self) -> None:
        """A dead session must archive itself and submit a new job, not lock the account."""
        with tempfile.TemporaryDirectory() as temporary:
            base = Path(temporary)
            personal = self.personal_root(base)
            paths = self.paths(personal)
            ...
            with mock.patch.object(MODULE, "run", return_value=purged):
                state, job = MODULE.session_status(metadata, "edu001", paths)

        self.assertEqual(state, "STALE")
        self.assertIsNone(job)
        self.assertNotIn(state, MODULE.ACTIVE_STATES | {"READY", "STARTING"})
